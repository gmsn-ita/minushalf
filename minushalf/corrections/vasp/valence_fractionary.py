...
of fractionary valence correction and optimizes the necessary parameters.
"""
import itertools
import os
import shutil
...
class VaspValenceFractionaryCorrection(Correction):
    """
    An algorithm that performs the correction in the orbitals
    atoms that contribute more than 5% in the VBM character
    """
    def __init__(
        self,
        root_folder: str,
    ...
    def _get_corrections_indexes(self) -> dict:
        ...
        for row, column in itertools.product(rows_name, columns_name):
            if self.vbm_projection[row][column] >= self.minimum_percentual:
                correction_indexes[row].append(
                    self.vbm_projection[column][row])
    ...
    def _find_cut(self, symbol: str, base_path: str) -> tuple:
        ...
        cut, eigenvalue = ternary_search(
            0,
            15,
            VaspValenceFractionaryCorrection.find_band_gap,
            **function_args,
        )
    ...
    def find_band_gap(
        ...
        VaspValenceFractionaryCorrection._write_potfile(
            cut,
            symbol,
        ...
        VaspValenceFractionaryCorrection._join_potfiles(
            base_path,
            default_potential_filename,
            atoms,
            potfiles_folder,
